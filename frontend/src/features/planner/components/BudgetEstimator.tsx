...
import { BudgetBreakdown } from "../types/planner"

interface BudgetEstimatorProps {
  budget: BudgetBreakdown
}

export function BudgetEstimator({ budget }: BudgetEstimatorProps) {
  ...
  const hotelPercentage = (budget.hotels / budget.total) * 100
  ...
  return (
    <div className="flex flex-col p-5 rounded-3xl border border-black/5 bg-white shadow-sm dark:border-white/5 dark:bg-slate-900/60 w-full select-none">
      ...
      <button
        ...
        className="w-full text-center py-2.5 rounded-xl border border-blue-500/10 hover:bg-blue-500/5 text-[10px] font-bold text-primary-blue dark:border-blue-500/20 dark:hover:bg-blue-500/10 dark:text-blue-400 transition-all cursor-pointer"
      >
